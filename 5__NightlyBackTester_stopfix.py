...
import sys
# Windows terminals default to cp1252, which crashes on the ticker-tape glyphs
# (arrows / box-drawing chars). Force UTF-8 so a long backtest never dies on a print.
try:
...
class StockSniperStrategy(bt.Strategy):
    ...
    # EXPERIMENTAL: append `signal_count` as a real strategy param (backtrader
    # rejects addstrategy kwargs that aren't declared here). `signal_count` =
    # how many signals get WRITTEN per day; `max_positions` = how many the sim
    # actually holds & sizes for. Decoupling them lets us flood the signals file
    # for FilterRubric to trim without shrinking each position. Handle both
    # dict-style and tuple-of-tuples STRATEGY_PARAMS.
    _base_params = tuple(STRATEGY_PARAMS.items()) if isinstance(STRATEGY_PARAMS, dict) else tuple(STRATEGY_PARAMS)
    params = _base_params + (
        ('signal_count', 20),
        # --- Diversify / beta-compression overlay (default OFF == byte-identical to legacy) ---
        # When `diversify` is True, process_buy_candidates reorders the prob-sorted
        # candidates with a greedy, diversity-aware selector that prefers names that
        # are (a) uncorrelated with what's already picked/held and (b) in
        # not-yet-represented correlation clusters. Pure A/B switch.
        # 2026-06-01 ablation (seed-42 clean A/B): correlation+cluster diversification
        # is a near-free win (return intact, Sortino 12.1->14.3, worst loss 443->340),
        # but the BETA penalty taxed the momentum alpha (beta-only run: 196%->143%).
        # So div_beta_weight now defaults to 0.0 -- bare --diversify == the winning
        # corr+cluster config. Re-enable the beta term explicitly via --div_beta_weight.
        ('diversify', False),
        ('div_corr_weight', 0.10),       # lambda on mean |corr| to already-picked set
        ('div_beta_weight', 0.0),        # penalty per +1 std of (high) rolling beta -- 0.0 = off (ablation winner)
        ('div_max_per_cluster', 1),      # max names per static cluster in the greedy pick (0 = no cap)
        ('beta_path', 'Data/RollingBeta.parquet'),
    )
    ...
    def __init__(self):
        ...
        # --- STOPFIX: real per-bar trailing stop with a hard catastrophe floor ---
        self.peak_price = {}        # data -> running peak (High) since entry
        self.active_stop = {}       # data -> the live resting protective sell order
        self._stop_kind = {}        # data -> 'Trailing Stop' | 'Hard Stop'
        self.exit_reason_override = {}  # data -> reason string for the next recorded exit
        self.hard_stop_pct = float(os.environ.get('SWEEP_HARDSTOP', '100'))  # catastrophe floor %; 100 == effectively off
        self.trail_pct = float(os.environ.get('SWEEP_TRAIL', '3.0'))         # trailing stop %
        logging.info(f"STOPFIX params: hard_stop={self.hard_stop_pct}%  trail={self.trail_pct}%")
        ...
        self.trade_recorder = TradeRecorder(os.environ.get('SWEEP_OUT', 'trade_history_stopfix.parquet'))  # STOPFIX: redirect (per-config via SWEEP_OUT), do NOT clobber live trade_history.parquet
        ...
        # --- Rolling-beta lookup for the diversify overlay (only loaded when enabled) ---
        # dict: ticker -> pd.Series(Beta) indexed by Date (sorted), looked up as-of.
        self.beta_lookup = {}
        if getattr(self.p, 'diversify', False):
            beta_path = getattr(self.p, 'beta_path', 'Data/RollingBeta.parquet')
            try:
                bdf = pd.read_parquet(beta_path)
                bdf['Date'] = pd.to_datetime(bdf['Date'])
                for tk, g in bdf.groupby('Ticker'):
                    self.beta_lookup[tk] = g.set_index('Date')['Beta'].sort_index()
                logging.info(f"DIVERSIFY: loaded rolling beta for {len(self.beta_lookup)} tickers from {beta_path}")
            except Exception as e:
                logging.warning(f"DIVERSIFY: could not load {beta_path} ({e}). "
                                f"Beta penalty disabled; correlation/cluster diversification still active.")

            # Fast O(1) lookup tables for select_diversified -- replaces the slow
            # per-candidate .loc calls that made the diversify path crawl.
            try:
                self._cluster_of = {t: int(c) for t, c in
                                    zip(self.correlation_df.index, self.correlation_df['Cluster'])}
                avail = [f"correlation_{k}" for k in range(self.total_groups)
                         if f"correlation_{k}" in self.correlation_df.columns]
                self._corr_col_idx = {int(c.split('_')[1]): j for j, c in enumerate(avail)}
                self._corr_mat = self.correlation_df[avail].to_numpy(dtype=float) if avail else None
                self._corr_row_idx = {t: i for i, t in enumerate(self.correlation_df.index)}
                logging.info(f"DIVERSIFY: built fast lookup ({self._corr_mat.shape if self._corr_mat is not None else None})")
            except Exception as e:
                self._cluster_of, self._corr_mat = None, None
                logging.warning(f"DIVERSIFY: fast-lookup build failed ({e}); falling back to .loc path.")
        ...
        # PERF (2026-06-01): the live-trading signal exports in
        # process_buy_candidates only need to run on the final backtest bar
        # (they overwrite each other daily and never affect simulated P&L).
        # Precompute the last date present across all feeds so we can detect
        # that bar cheaply. Falls back to the real last trading date.
        try:
            self._final_bt_date = max(
                pd.Timestamp(d.p.dataname['Date'].max()).date() for d in self.datas
            )
            logging.info(f"PERF: live-signal exports gated to final backtest bar {self._final_bt_date}")
        except Exception as e:
            logging.warning(f"Could not compute final backtest date ({e}); "
                            f"falling back to last_trading_date for export gating")
            self._final_bt_date = self.last_trading_date
        ...
        # --- compact ticker-tape output state ---
        import shutil
        _term_w = shutil.get_terminal_size(fallback=(80, 24)).columns
        self._day_chips = []            # (colored, visible) chips for the current day
        self._last_flush_equity = None  # equity at last tape flush, for Δday
        self._tape_width = max(60, _term_w)   # auto-detected render width for the tape

        self.progress_bar = tqdm(
            total=self.total_bars,
            desc="Backtest",
            unit="d",
            ncols=max(80, _term_w),
    ...
    def next(self):
        ...
        # STOPFIX: short-proof intrabar stop check FIRST; it self.close()s any
        # position whose Low breached the hard floor / trailing level this bar.
        stopped = self.check_protective_stops()

        # Continue with normal position management (skip names already stopped out)
        sell_data = [d for d in self.datas if self.getposition(d).size > 0 and d not in stopped]
        for d in sell_data:
        ...
        # Flush this day's compact ticker tape (only days with closed trades print)
        self._flush_day_tape()
    ...
    def _flush_day_tape(self):
        """Render the current backtest day's closed trades as a compact, color-coded
        ticker tape plus a one-line summary (Sigma total, Sharpe, delta-day). Routed
        through the progress bar's write() so the fixed bottom bar is never corrupted.
        Days with no closed trades print nothing."""
        if not self._day_chips:
            return
        GREEN, RED, DIM, RESET = "\033[32m", "\033[31m", "\033[2m", "\033[0m"
        if self._last_flush_equity is None:
            self._last_flush_equity = self.broker.startingcash

        # Pack colored chips into wrapped lines using visible (ANSI-free) widths.
        lines, row, row_len = [], [], 0
        for colored, visible in self._day_chips:
            extra = len(visible) + (2 if row else 0)
            if row and row_len + extra > max(20, self._tape_width - 2):
        ...
        total_ret = (cur_equity / self.broker.startingcash - 1) * 100
        dday = ((cur_equity / self._last_flush_equity - 1) * 100) if self._last_flush_equity else 0.0
        self._last_flush_equity = cur_equity

        n = len(self.trade_pct_returns)
        if n >= 2:
            arr = np.array(self.trade_pct_returns)
            sd = arr.std(ddof=1)
            sharpe = (arr.mean() / sd) * (n ** 0.5) if sd > 0 else 0.0
        else:
            sharpe = 0.0

        tcol, dcol = (GREEN if total_ret >= 0 else RED), (GREEN if dday >= 0 else RED)
        tsign, dsign = ("+" if total_ret >= 0 else ""), ("+" if dday >= 0 else "")
        sep = f"{DIM}│{RESET}"
        # Day block: header rule (date), railed chip rows, summary footer.
        date_str = str(self.datetime.date())
        header = f"{DIM}┌─ {date_str} {'─' * max(0, self._tape_width - len(date_str) - 4)}{RESET}"
        footer = (f"{DIM}└─{RESET} "
                  f"Σ {tcol}{tsign}{total_ret:.2f}%{RESET}  {sep}  "
                  f"{DIM}Sharpe{RESET} {sharpe:.2f}  {sep}  "
                  f"Δday {dcol}{dsign}{dday:.2f}%{RESET}  {sep}  "
                  f"{DIM}n={n}{RESET}")

        self.progress_bar.write(header)
        for ln in lines:
            self.progress_bar.write(f"{DIM}│{RESET} {ln}")
        self.progress_bar.write(footer)
        self._day_chips = []
    ...
    def get_buy_candidates(self, current_date):
        ...
        for d in self.datas:
            ...
            # SUPER CHEAP PREFILTER
            try:
                if d.UpProbability[0] < 0.30:
                    continue
            except Exception:
                continue
    ...
    def can_buy_may_17th_eda_attempt_FAILED(self, data, current_date):
        ...
        Run that script against trade_history.parquet + Data/RFpredictions/ and
    ...
    def _is_final_export_bar(self, current_date):
        """True only on the last bar of the backtest -- the only bar on which
        the live-trading signal exports need to be written (each daily write
        just overwrites the prior one and none of them affect simulated P&L).
        See process_buy_candidates for the full rationale."""
        try:
            return current_date >= self._final_bt_date
        except Exception:
            return True  # fail open: never silently drop the live export

    def process_buy_candidates(self, buy_candidates, current_date, verbose=False):
        """Process buy candidates and execute trades.

        PERF (2026-06-01): the live-trading signal EXPORTS in this method
        (force_best_signal_for_current_day + save_best_buy_signals +
        save_guaranteed_signals_to_parquet) used to run on EVERY simulated
        bar. They are pure side-effects -- a FinBERT model load + finviz
        network scrape + parquet read/write/read-back + NYSE-calendar builds
        -- that the live trader only ever consumes from the FINAL bar (each
        daily write just overwrites the previous one), so running them ~252x
        was the dominant cost of a full-year backtest with ZERO effect on
        simulated P&L. They are now gated to the final backtest bar only via
        _is_final_export_bar(). Trade sorting / diversify / execution still
        run every day, so backtest results are byte-identical.
        """
        export_today = False  # BASELINE-EXITS: disable live-signal exports (no finviz/FinBERT, no clobber); sim P&L unaffected
        ...
        # Diversify overlay (A/B toggle): reorder so the *traded* front of the
        # list is uncorrelated / lower-beta / spread across clusters. OFF by
        # default -> this block is skipped and behaviour is identical to legacy.
        if getattr(self.p, 'diversify', False):
            buy_candidates = self.select_diversified(buy_candidates, current_date)

        # --- Live-trading signal exports: FINAL BAR ONLY (see docstring) ---
        if export_today:
            # Legacy format (for backwards compatibility)
            self.save_best_buy_signals(buy_candidates)

            # Consolidated format for live trading
            if buy_candidates:
                signals = []
                ...
                for d, size, correlation in buy_candidates[:self.p.signal_count]:
                    price = d.close[0]
                ...
                try:
                    save_guaranteed_signals_to_parquet(signals, next_trading_day)
                except Exception as e:
                    logging.error(f"Error saving to consolidated format: {str(e)}")
    ...
    # ==================================================================
    # Diversify / beta-compression overlay helpers (active only when
    # self.p.diversify is True). All read-only; legacy path untouched.
    # ==================================================================
    def get_cluster(self, ticker):
        """Static cluster id for a ticker, or None if unknown (O(1) dict lookup)."""
        c = getattr(self, '_cluster_of', None)
        if c is not None:
            return c.get(ticker)
        try:
            if ticker in self.correlation_df.index:
                return int(self.correlation_df.loc[ticker, 'Cluster'])
        except Exception:
            pass
        return None

    def get_beta(self, ticker, current_date):
        """As-of rolling beta for `ticker` on/just-before `current_date`.

        Returns np.nan if no beta is available (missing ticker or not enough
        history). Lookahead-safe: only uses the most recent beta dated <= today.
        """
        s = self.beta_lookup.get(ticker)
        if s is None or len(s) == 0:
            return np.nan
        try:
            pos = s.index.searchsorted(pd.Timestamp(current_date), side='right') - 1
            if pos < 0:
                return np.nan
            val = s.iloc[pos]
            return float(val) if val == val else np.nan  # NaN guard
        except Exception:
            return np.nan

    def mean_corr_to_set(self, candidate_ticker, tickers):
        """Mean |correlation| of a candidate to an arbitrary set of tickers.

        Generalises get_mean_correlation to any set (already-picked + held),
        reusing the per-cluster correlation columns as the relatedness proxy.
        """
        if not tickers:
            return 0.0
        # Fast path: precomputed numpy matrix + dict indices (O(len(tickers))).
        mat = getattr(self, '_corr_mat', None)
        if mat is not None:
            ri = self._corr_row_idx.get(candidate_ticker)
            if ri is None:
                return 0.0
            row = mat[ri]
            vals = []
            for pos in tickers:
                if pos == candidate_ticker:
                    continue
                cl = self._cluster_of.get(pos)
                if cl is None:
                    continue
                cj = self._corr_col_idx.get(cl)
                if cj is not None:
                    vals.append(abs(row[cj]))
            return float(np.mean(vals)) if vals else 0.0
        # Fallback: original .loc path.
        if candidate_ticker not in self.correlation_df.index:
            return 0.0
        row = self.correlation_df.loc[candidate_ticker]
        vals = []
        for pos in tickers:
            if pos == candidate_ticker or pos not in self.correlation_df.index:
                continue
            try:
                pcl = self.correlation_df.loc[pos, 'Cluster']
                col = f"correlation_{pcl}"
                if col in self.correlation_df.columns:
                    vals.append(abs(float(row[col])))
            except Exception:
                continue
        return float(np.mean(vals)) if vals else 0.0

    def select_diversified(self, sorted_candidates, current_date):
        """Greedy, diversity-aware reorder of prob-sorted buy candidates.

        At each step pick the candidate maximising:
            UpProbability
              - div_corr_weight * mean|corr| to already-picked + held
              - div_beta_weight * max(0, beta_zscore)        # only HIGH beta penalised
        subject to a per-cluster count cap (div_max_per_cluster). Candidates
        blocked solely by the cluster cap are appended (in their original prob
        order) after the diversified picks, so the written signals file keeps its
        depth -- only the *front* of the list (what actually gets traded) changes.

        Returns a reordered list of the same (data, size, corr) tuples.
        """
        if not sorted_candidates:
            return sorted_candidates

        # 1. HARD GARBAGE FILTER (cheap O(n)): drop junk before the greedy ever
        # sees it. can_buy already enforces most of this on the legacy path; this
        # is the diversify-path guard the user asked for, and it keeps the greedy
        # working set clean.
        HARD_MIN_PRICE, HARD_MAX_PRICE, HARD_MIN_PROB = 2.0, 1500.0, 0.30
        viable, rejected = [], []
        for c in sorted_candidates:
            d = c[0]
            try:
                px = float(d.close[0]); pr = float(d.UpProbability[0])
            except Exception:
                rejected.append(c); continue
            if px < HARD_MIN_PRICE or px > HARD_MAX_PRICE or pr < HARD_MIN_PROB:
                rejected.append(c)
            else:
                viable.append(c)

        # 2. CAP THE WORKING SET. Candidates are already prob-sorted and we only
        # trade max_positions of them, so scoring the long tail every day is pure
        # waste -- this is what made --diversify crawl. Bound it so per-day cost is
        # flat regardless of how many tickers --sample loads.
        cap = max(int(self.p.signal_count) * 4, 60)
        pool = viable[:cap]
        tail = viable[cap:]

        held = [d._name for d in self.datas if self.getposition(d).size > 0]
        max_per_cluster = self.p.div_max_per_cluster or 0

        # Pre-score betas across the (bounded) pool so the penalty is on a
        # comparable scale regardless of the day's absolute beta level.
        betas = {c[0]._name: self.get_beta(c[0]._name, current_date) for c in pool}
        bvals = [b for b in betas.values() if b == b]  # drop NaN
        bmean = float(np.mean(bvals)) if bvals else 0.0
        bstd = float(np.std(bvals)) if bvals and np.std(bvals) > 0 else 1.0

        # Index-based selection -- never compare the candidate tuples with ==,
        # because backtrader overloads comparison operators on data/line objects.
        picked_idx = []                                  # indices into pool, in pick order
        used = [False] * len(pool)
        picked_tickers = list(held)                      # correlate against existing holdings too
        cluster_counts = Counter(self.get_cluster(t) for t in held)

        while len(picked_idx) < self.p.signal_count:
            best_i, best_score = -1, float('-inf')
            for i, cand in enumerate(pool):
                if used[i]:
                    continue
                tk = cand[0]._name
                cl = self.get_cluster(tk)
                if max_per_cluster and cl is not None and cluster_counts.get(cl, 0) >= max_per_cluster:
                    continue
                try:
                    prob = float(cand[0].UpProbability[0])
                except Exception:
                    prob = 0.0
                corr_pen = self.mean_corr_to_set(tk, picked_tickers)
                b = betas.get(tk, np.nan)
                beta_z = 0.0 if (b != b) else (b - bmean) / bstd
                score = (prob
                         - self.p.div_corr_weight * corr_pen
                         - self.p.div_beta_weight * max(0.0, beta_z))
                if score > best_score:
                    best_score, best_i = score, i
            if best_i < 0:
                break  # everything left is cluster-capped
            used[best_i] = True
            picked_idx.append(best_i)
            bt_name = pool[best_i][0]._name
            picked_tickers.append(bt_name)
            bcl = self.get_cluster(bt_name)
            cluster_counts[bcl] = cluster_counts.get(bcl, 0) + 1

        # Diversified front -> cluster-capped leftovers (prob order) -> untouched
        # tail -> hard-rejected junk last (never traded; only fills the signal
        # file if nothing better exists).
        picked = [pool[i] for i in picked_idx]
        leftovers = [pool[i] for i in range(len(pool)) if not used[i]]
        return picked + leftovers + tail + rejected
    ...
    def execute_buy_with_bracket(self, data, size, current_date):
        """STOPFIX (2026-06-01): the old buy_bracket()/StopTrail child never fired --
        the 2-4 day momentum exit closed every position and cancelled the bracket
        first, so sweeping the trail changed *nothing* (identical to the penny).
        Replaced with a plain market buy; a REAL trailing stop with a hard floor is
        managed bar-by-bar in update_trailing_stops() (resting Stop order the broker
        fills intrabar against the actual Low / at the open on a gap-through).
        Levels read from env: SWEEP_HARDSTOP (catastrophe floor %), SWEEP_TRAIL
        (trailing %). No take-profit (matches the live hard-stop + trailing setup)."""
        symbol = data._name
        ...
        if main_order:
            self.entry_prices[data] = current_price       # provisional; corrected to actual fill in handle_buy_execution
    ...
    def check_protective_stops(self):
        """STOPFIX: short-proof intrabar stop check. Resting Stop/StopTrail orders
        kept double-filling into shorts (uncancelled trails accumulating across
        re-entries), so instead we detect the breach ourselves and exit with
        self.close(), which flattens EXACTLY the held size and can never short.

        For each held position: the trailing level uses the peak through the PRIOR
        bar (no intrabar lookahead); if THIS bar's Low breaches max(hard floor,
        trailing level) the position is flattened (market -> next bar's open, a
        conservative fill) and labelled. Returns the set closed this bar so the
        momentum logic skips them. Levels from SWEEP_TRAIL / SWEEP_HARDSTOP
        (>=100 == that leg off)."""
        stopped = set()
        if self.hard_stop_pct >= 100 and self.trail_pct >= 100:
            return stopped
        for data in self.datas:
            ...
            if not entry:
                continue
            prior_peak = self.peak_price.get(data, entry)
            hard_level = entry * (1.0 - self.hard_stop_pct / 100.0)
            trail_level = prior_peak * (1.0 - self.trail_pct / 100.0)
            desired = max(hard_level, trail_level)
            if float(data.low[0]) <= desired:
                self.exit_reason_override[data] = (
                    'Hard Stop' if (self.hard_stop_pct < 100 and hard_level >= trail_level)
                    else 'Trailing Stop')
                self.close(data=data)
                self.peak_price.pop(data, None)
                stopped.add(data)
            else:
                # not stopped -> fold this bar's High into the running peak for next bar
                self.peak_price[data] = max(prior_peak, float(data.high[0]))
        return stopped
    ...
    def exit_position(self, data):
        ...
        # STOPFIX: cancel the resting trailing/hard stop so it can't later sell us short
        st = self.active_stop.pop(data, None)
        if st is not None and st.alive():
            self.cancel(st)
        self._stop_kind.pop(data, None)
        self.peak_price.pop(data, None)
    ...
    def save_best_buy_signals(self, buy_candidates):
        ...
        # Create a fresh DataFrame for new signals
        signal_data = []
        candidates_to_process = buy_candidates[:self.p.signal_count]
    ...
    def handle_buy_execution(self, order):
        ...
        # STOPFIX: seed the running peak at the actual fill; check_protective_stops()
        # trails it and exits intrabar on a breach.
        self.peak_price[data] = order.executed.price
    ...
    def handle_sell_execution(self, order):
        ...
        if self.getposition(data).size == 0:
            ...
            # STOPFIX: the exit reason (Trailing/Hard Stop) is set at breach-detection
            # time in check_protective_stops(); determine_exit_reason() consumes it.
            self.peak_price.pop(data, None)
            ...
            if entry_price is not None:
                ...
                # Buffer a colored chip for this trade; the day's tape + summary is
                # flushed via the progress bar's write() at the end of next().
                _GREEN, _RED, _RESET = "\033[32m", "\033[31m", "\033[0m"
                _arrow = "▲" if profit_pct >= 0 else "▼"
                _sign = "+" if profit_pct >= 0 else ""
                _visible = f"{_arrow} {symbol} {_sign}{profit_pct:.2f}%"
                _col = _GREEN if profit_pct >= 0 else _RED
                self._day_chips.append((f"{_col}{_visible}{_RESET}", _visible))
                ...
                # add_completed_trade(completed_trade_data)  # BASELINE-EXITS: neutralized (appends to live Data/TradeHistory.parquet)

                is_loss_for_tracking = profit_abs < 0
                # update_trade_result(symbol, is_loss_for_tracking, exit_price, exit_date)  # BASELINE-EXITS: neutralized (writes _Buy_Signals.parquet)
    ...
    def determine_exit_reason(self, data):
        ...
        # STOPFIX: a resting-stop fill set an authoritative reason -- use & clear it.
        ov = self.exit_reason_override.pop(data, None)
        if ov:
            return ov
        current_price = data.close[0]
    ...
    def stop(self):
        # STOPFIX invariant: warn only if the short/slot desync ever reappears
        shorts = [(d._name, self.getposition(d).size) for d in self.datas if self.getposition(d).size < 0]
        if shorts:
            print(f"STOPFIX-WARN: {len(shorts)} SHORT positions leaked: {shorts[:6]} (open_positions={self.open_positions})")
        self._flush_day_tape()
...
def save_guaranteed_signals_to_parquet(signals, next_trading_day=None):
    ...
    try:
        ...
        # audit columns into the dataframe BEFORE the single final write
        # (annotate only, no drops).
...
def setup_backtest_environment(args, logger):
    ...
    # STOPFIX: cap at 4 cores (user is gaming in the background; keep headroom)
    ...
    # EXPERIMENTAL: forward --max_positions into the strategy. In the original
    # backtester this arg was parsed but never wired in, so the live cap was
    # always STRATEGY_PARAMS' max_positions (4). Every signal-writing path slices
    # buy_candidates[:self.p.max_positions], so this also controls how many
    # signals get written to Data/0__Signals.parquet + _Buy_Signals.parquet
    # before the FilterRubric step trims them.
    if getattr(args, 'max_positions', None) is not None:
        strategy_params['max_positions'] = args.max_positions
        logger.info(f"EXPERIMENTAL: max_positions overridden to {args.max_positions}")

    if getattr(args, 'signal_count', None) is not None:
        strategy_params['signal_count'] = args.signal_count
        logger.info(f"EXPERIMENTAL: signal_count (signals written/day) set to {args.signal_count}")

    # --- Diversify / beta-compression overlay (A/B toggle) ---
    if getattr(args, 'diversify', False):
        strategy_params['diversify'] = True
        if getattr(args, 'div_corr_weight', None) is not None:
            strategy_params['div_corr_weight'] = args.div_corr_weight
        if getattr(args, 'div_beta_weight', None) is not None:
            strategy_params['div_beta_weight'] = args.div_beta_weight
        if getattr(args, 'div_max_per_cluster', None) is not None:
            strategy_params['div_max_per_cluster'] = args.div_max_per_cluster
        if getattr(args, 'beta_path', None) is not None:
            strategy_params['beta_path'] = args.beta_path
        logger.info(
            "DIVERSIFY ENABLED: corr_weight=%s beta_weight=%s max_per_cluster=%s beta_path=%s"
            % (strategy_params.get('div_corr_weight', 0.10),
               strategy_params.get('div_beta_weight', 0.0),
               strategy_params.get('div_max_per_cluster', 1),
               strategy_params.get('beta_path', 'Data/RollingBeta.parquet')))
...
def select_data_files(args, data_dir, logger):
    ...
    if args.sample > 0:
        ...
        # Deterministic sampling for clean A/B testing: with --seed, control and
        # treatment runs draw the IDENTICAL ticker subset, so any metric delta is
        # attributable to the strategy change, not to which 90% got sampled.
        _seed = getattr(args, 'seed', None)
        if _seed is not None:
            random.seed(_seed)
            np.random.seed(_seed)
            logger.info(f"Sampling seeded with --seed {_seed} (reproducible subset)")
...
def arg_parser():
    ...
    parser.add_argument("--max_positions", type=int, default=4, help="Concurrent positions the SIM holds & sizes for (default 4, matches live deployment). Decoupled from signals written -- see --signal_count.")
    parser.add_argument("--signal_count", type=int, default=20, help="How many signals get WRITTEN each day to Data/0__Signals.parquet + _Buy_Signals.parquet (the pool FilterRubric trims). Default 20. Independent of --max_positions, so it does NOT affect sim position sizing.")
    ...
    # --- Diversify / beta-compression overlay (A/B toggle, default OFF) ---
    parser.add_argument("--diversify", action='store_true', default=False,
                        help="Enable greedy diversity-aware selection (uncorrelated + lower-beta + unique-cluster). "
                             "OFF by default == identical to the legacy 220%% run. Requires Data/RollingBeta.parquet "
                             "for the beta term (run compute_rolling_beta.py first); correlation/cluster diversification "
                             "still works without it.")
    parser.add_argument("--div_corr_weight", type=float, default=None,
                        help="Lambda penalty on mean |correlation| to already-picked names (default 0.10).")
    parser.add_argument("--div_beta_weight", type=float, default=None,
                        help="Penalty per +1 std of (high) rolling beta. Default 0.0 (OFF) -- the 2026-06-01 "
                             "ablation showed the beta term taxes the momentum alpha (196%%->143%% beta-only). "
                             "Pass e.g. 0.02-0.05 to re-enable it.")
    parser.add_argument("--div_max_per_cluster", type=int, default=None,
                        help="Max names per static correlation cluster in the greedy pick (default 1; 0 disables the cap).")
    parser.add_argument("--beta_path", type=str, default=None,
                        help="Path to rolling-beta parquet (default Data/RollingBeta.parquet).")
    parser.add_argument("--seed", type=int, default=None,
                        help="Seed for --sample ticker selection. Pass the SAME --seed to control and treatment "
                             "runs so they backtest the identical subset (required for a clean diversify A/B "
                             "when --sample < 100). Omit for the legacy non-deterministic behaviour.")
    return parser.parse_args()



if __name__ == "__main__":
    # clear_completed_trades()  # BASELINE-EXITS: neutralized (wipes live Data/TradeHistory.parquet)
    main()
