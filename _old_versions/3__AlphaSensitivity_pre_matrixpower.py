...
def compute_gp_input_features(df, window=60):
    ...
    df['Return']    = ret
    df['High_Low']  = (high - low) / (close + 1e-10)
    df['High_Close'] = (high - prev_c) / (prev_c + 1e-10)
    df['Low_Close']  = (low  - prev_c) / (prev_c + 1e-10)

    # time_reversal_asymmetry: mean((x_{t}^2 * x_{t-1}) - (x_{t-1}^2 * x_t))
    df['time_reversal_asymmetry'] = (
    ...
    df['time_between_extremes'] = close.rolling(window, min_periods=min_p).apply(_tbe, raw=True)
    ...
    df['volume_entropy'] = volume.rolling(window, min_periods=min_p).apply(_vol_ent, raw=True)

    # information_rate_of_change: mean absolute first difference of returns
    df['information_rate_of_change'] = ret.diff().abs().rolling(window, min_periods=min_p).mean()
    ...
    df['emd_features_IMF_1_Energy'] = (imf1 ** 2).rolling(window, min_periods=min_p).mean()
    df['emd_features_IMF_1_Std']    = imf1.rolling(window, min_periods=min_p).std()
    df['emd_features_IMF_2_Energy'] = (imf2 ** 2).rolling(window, min_periods=min_p).mean()
    df['emd_features_IMF_2_Std']    = imf2.rolling(window, min_periods=min_p).std()
    ...
    df['entropy'] = ret.rolling(window, min_periods=min_p).apply(_ent, raw=True)
    ...
    df['noise_WhiteNoise'] = np.exp(-(slopes ** 2) / 0.5)          # peak at slope=0
    df['noise_PinkNoise']  = np.exp(-((slopes + 1.0) ** 2) / 0.5)  # peak at slope=-1
    ...
    def _higuchi(x, kmax=6):
        ...
        return float(slope)
    df['higuchi_fractal_dimension'] = close.rolling(window, min_periods=min_p).apply(
        _higuchi, raw=True
    )
    ...
    df['lyapunov_exponent'] = close.rolling(window, min_periods=min_p).apply(_lyap, raw=True)
    ...
    df['dfa_alpha'] = ret.rolling(window, min_periods=min_p).apply(_dfa, raw=True)
    ...
    df['spectral_entropy'] = ret.rolling(window, min_periods=min_p).apply(_spec_ent, raw=True)
    ...
    df['autocorrelation'] = ret.rolling(window, min_periods=min_p).apply(_ac1, raw=True)

    # hurst_exponent: reuse existing rolling implementation
    df['hurst_exponent'] = rolling_hurst_exponent(close, window_size=window)

    df['higher_order_moments_Skewness'] = ret.rolling(window, min_periods=min_p).skew()
    df['higher_order_moments_Kurtosis'] = ret.rolling(window, min_periods=min_p).kurt()
...
def add_gp_cross_sectional_features(output_dir):
    ...
    if not files:
        logging.warning("GP cross-sectional pass: no processed files found")
        return

    logging.info(f"GP cross-sectional pass: loading {len(files)} files...")
    dfs = []
    for fp in files:
        try:
            ...
            logging.warning(f"GP pass: could not load {fp}: {e}")

    if not dfs:
        logging.warning("GP cross-sectional pass: no files loaded successfully")
        return
    ...
    if missing:
        logging.warning(f"GP cross-sectional pass: missing columns {missing}, skipping")
        return

    logging.info("Computing GP cross-sectional features (F1-N4)...")
    try:
        gp_feats = compute_gp_features(combined, date_col='Date')
    except Exception as e:
        logging.error(f"GP cross-sectional pass failed: {e}")
        return
    ...
    # Write GP features back to each ticker's file
    for fp in files:
        mask = combined['_gp_src'] == fp
        subset = combined.loc[mask].drop(columns=['_gp_src']).reset_index(drop=True)
        try:
            subset.to_parquet(fp, index=False)
        except Exception as e:
            logging.warning(f"GP pass: failed to save {fp}: {e}")

    logging.info(f"GP features saved to {len(files)} files: {gp_cols}")
...
def calculate_vix_features(df):    
    ...
    result_df['VIX_MA10'] = result_df['VIX_Close'].shift(1).rolling(10, min_periods=10).mean()
    # VIX_MA20, VIX_MA50, VIX_Cross_10_50, VIX_Cross_20_50 removed (weak/redundant)
    ...
    for window in [20, 50, 100]:
        if len(result_df) >= window:
            ...
            # VIX_Z_20d removed (redundant with vix_zscore which uses proper lag)
            if window != 20:
    ...
    # 8. RISK ADJUSTMENT
    # VIX_Risk_Multiplier removed (redundant with VIX_Close, just VIX_Close/20)
...
def calculate_moving_average_indicators(df):
    ...
    # ATR% removed (redundant with atr_percentage in calculate_volatility_atr_features)
...
def calculate_price_volume_indicators(df):
    ...
    # Validated in 10_AccumulationSignalSqueezer.ipynb
    # acc_pvt_z       : U-shaped — BOTH high AND low extremes bearish (~1-2% days, edge -7% to -10%)
    # acc_pvt_extreme : |pvt_z| — actionable form; exclude when high
    # acc_composite_z : low values bullish (~10-15% days, edge +2%)
    # acc_pvt_bear_conf  : confidence distance beyond |pvt_z| = 1.0
    # acc_comp_bull_conf : confidence distance beyond composite_z = -1.25

    # -- PVT slope, z-scored vs trailing 252-day distribution --
    _pvt       = (close.pct_change() * volume.clip(lower=1)).cumsum()
    _vol_ma20  = volume.shift(1).rolling(20, min_periods=5).mean().clip(lower=1)
    _pvt_slope = _pvt.diff(20) / _vol_ma20
    _pvt_mu    = _pvt_slope.shift(1).rolling(252, min_periods=60).mean()
    _pvt_sd    = _pvt_slope.shift(1).rolling(252, min_periods=60).std().clip(lower=1e-8)
    _pvt_z     = ((_pvt_slope - _pvt_mu) / _pvt_sd).clip(-5, 5)
    new_columns['acc_pvt_z']       = _pvt_z
    new_columns['acc_pvt_extreme'] = _pvt_z.abs()

    # -- Composite z-score: avg rolling pct-rank of (CMF-20, up-vol-ratio, OBV-slope) --
    _hl        = (high - low).clip(lower=1e-8)
    _clv       = ((close - low) - (high - close)) / _hl
    _vol       = volume.clip(lower=1)
    _vol20_sum = _vol.rolling(20, min_periods=5).sum().clip(lower=1)
    _cmf20     = (_clv * _vol).rolling(20, min_periods=5).sum() / _vol20_sum
    _up_day    = (close > close_shift_1).astype(float)
    _uvr20     = (_vol * _up_day).rolling(20, min_periods=5).sum() / _vol20_sum
    _obv       = pd.Series(
        np.where(close > close_shift_1.fillna(close), _vol, -_vol).cumsum(),
        index=df.index
    )
    _obv_slope = _obv.diff(20) / _vol_ma20
    _obv_mu    = _obv_slope.shift(1).rolling(252, min_periods=60).mean()
    _obv_sd    = _obv_slope.shift(1).rolling(252, min_periods=60).std().clip(lower=1e-8)
    _obv_z     = ((_obv_slope - _obv_mu) / _obv_sd).clip(-5, 5)

    def _prank(s):
        return s.rolling(120, min_periods=20).rank(pct=True)

    _raw_comp  = (_prank(_cmf20) + _prank(_uvr20) + _prank(_obv_z)) / 3.0
    _comp_mu   = _raw_comp.shift(1).rolling(252, min_periods=60).mean()
    _comp_sd   = _raw_comp.shift(1).rolling(252, min_periods=60).std().clip(lower=1e-8)
    _comp_z    = ((_raw_comp - _comp_mu) / _comp_sd).clip(-5, 5)
    new_columns['acc_composite_z'] = _comp_z

    # -- Confidence scores: distance beyond empirical signal thresholds --
    new_columns['acc_pvt_bear_conf']  = (_pvt_z.abs() - 1.0).clip(lower=0)   # |pvt_z| > 1.0  → bearish
    new_columns['acc_comp_bull_conf'] = (-_comp_z - 1.25).clip(lower=0)       # comp_z < -1.25 → bullish
...
def calculate_genetic_indicators(df):
    ...
    # G_Price_Differential_Analyzer removed (redundant with Price_Differential_Ratio)
...
def calculate_kalman_indicators(df):
    try:
        ...
        # Distance to Resistance (%) removed (weak feature)
...
def calculate_volatility_indicators(df):
    ...
    def add_multiple_mean_reversion_z_scores(data, price_column='Smoothed_Close', windows=[90, 151], std_multipliers=[1, 3]):
...
def calculate_significant_indicators(df):
    ...
    # Normalize Pattern_Indicator (HC_Ratio_norm and Significant_Indicators_Ensemble removed as redundant)
    sig_indicators = ['Pattern_Indicator', 'HC_Predict_Regime']
...
def add_trading_signal_features(df):
    ...
    # === CANDLE STRUCTURE FEATURES (new — validated unique vs existing features) ===
    features.update(calculate_candle_structure_features(df))
...
def calculate_vix_regime_features(df):
    ...
    # VIX momentum and acceleration (vix_momentum_5d removed, redundant with VIX_Change_5d)
    _vix_momentum_5d = current_vix.pct_change(5)
    features['vix_acceleration'] = _vix_momentum_5d.diff()
...
def calculate_liquidity_features(df):
    ...
    # Current dollar volume relative measures (dollar_volume_ratio_10d removed, weak)
...
def calculate_price_action_features(df):
    ...
    # Intraday range characteristics (intraday_range_pct removed, redundant with percent_range)
    ...
    # Price action patterns (doji_pattern removed, weak)
...
def calculate_candle_structure_features(df):
    """
    Eight new candle-structure and volatility features validated against
    ProcessedData existing features (max correlation < 0.70 vs any existing col).

    Features added:
      lower_wick_ratio        – lower wick / total range  (UP→DOWN signal)
      upper_wick_ratio        – upper wick / total range  (UP→UP  signal)
      wick_asymmetry          – upper_wick_ratio - lower_wick_ratio
      signed_volume           – Volume × sign(Close - Open)
      gap_fill_signal         – normalised overnight gap + normalised intraday return
      upside_vol_percentile   – rolling 50d percentile rank of upside-only volatility
      days_since_extreme_ret  – log1p(days since |return| > 2σ), clipped at 100
      volatility_entropy      – Shannon entropy of abs-return magnitude distribution
    """
    features = {}
    ...
    features['wick_asymmetry']   = features['upper_wick_ratio'] - features['lower_wick_ratio']

    # ── signed volume ──────────────────────────────────────────────────────────
    features['signed_volume'] = V * np.sign(C - O)

    # ── gap fill signal ────────────────────────────────────────────────────────
    # (overnight_return / overnight_vol) + (intraday_return / intraday_vol)
    # Captures: large gap that was reversed intraday (fade) or continued (breakout)
    log_co  = np.log(C / (O + EPS))
    log_opc = np.log(O / (C.shift(1) + EPS))
    intraday_vol  = log_co.rolling(20, min_periods=10).std().replace(0, np.nan)
    overnight_vol = log_opc.rolling(20, min_periods=10).std().replace(0, np.nan)
    features['gap_fill_signal'] = (log_opc / overnight_vol) + (log_co / intraday_vol)

    # ── upside vol percentile ──────────────────────────────────────────────────
    ...
    upside_vol_series = log_ret.rolling(20, min_periods=10).apply(_upside_vol, raw=True)
    ...
    features['upside_vol_percentile'] = upside_vol_series.fillna(
        upside_vol_series.median()
    ).rolling(50, min_periods=25).apply(_pct_rank, raw=True)

    # ── days since extreme return ──────────────────────────────────────────────
    ret_mu  = log_ret.rolling(50, min_periods=25).mean()
    ret_sig = log_ret.rolling(50, min_periods=25).std()
    is_ext  = ((log_ret - ret_mu).abs() > 2 * ret_sig).astype(float)

    dse  = np.full(len(df), np.nan)
    last = -1
    for i in range(len(is_ext)):
        if is_ext.iloc[i] == 1.0:
            last = i
        if last >= 0:
            dse[i] = i - last
    features['days_since_extreme_ret'] = np.log1p(
        pd.Series(dse, index=df.index).clip(upper=100)
    )
    ...
    def _vol_entropy(x):
        ...
        if sigma == 0:
            return 0.0
        # Fixed sigma-based bins: measures heavy-tail vs thin-tail shape
    ...
    features['volatility_entropy'] = log_ret.abs().rolling(20, min_periods=10).apply(
        _vol_entropy, raw=True
    )

    return features


def compute_intraday_regime_score(df: pd.DataFrame) -> pd.DataFrame:
    EPS = 1e-8
    """
    Compute the Intraday Regime Score v2 for a single ticker.

    Parameters
    ----------
    df : pd.DataFrame
        Daily OHLCV data with columns: Date, Open, High, Low, Close, Volume.
        Rows may be in any order — the function sorts by Date internally.

    Returns
    -------
    pd.DataFrame
        Input DataFrame with two columns added:
          - irs_v2        : final score ∈ [-1, +1]
          - irs_extremity : move-size extremity ∈ [0, 1]

    Notes
    -----
    All intermediate series are computed inline; nothing is mutated on the
    caller's DataFrame (a copy is made internally).

    Formula outline
    ---------------
    Step 1  Move-size extremity
            move_pct  = rolling_rank( |open_close_ratio|.shift(1), 252 )
            extremity = clip( (move_pct - 0.75) / 0.15, 0, 1 )

    Step 2  Regime-switched candle signal
            cir_c = close_in_range * 2 - 1
            core  = cir_c * (2 * extremity - 1)
            Moderate (ext=0): core = -cir_c  → high CIR = reversal DOWN
            Extreme  (ext=1): core = +cir_c  → high CIR = continuation UP

    Step 3  Upper-wick additive (independent IC ≈ +0.041)
            uw_z = zscore(upper_wick_frac, 252).clip(-2.5, 2.5) / 2.5
            raw  = core * 0.85 + uw_z * 0.15

    Step 4  Trend amplitude  (weakens in strong uptrends, never flips sign)
            trend_amp = 1.0 - 0.30 * (trend_pct - 0.5)   ∈ [0.85, 1.15]

    Step 5  Volume amplitude (U-shaped: both extreme-low and extreme-high amplify)
            vol_amp = 1.0 + 0.20 * (vol_pct - 0.5)^2 * 4   ∈ [1.0, 1.20]

    Step 6  Normalise
            amplified = raw * trend_amp * vol_amp
            irs_v2    = zscore(amplified, 252).clip(-3, 3) / 3
    """
    df = df.copy()
    df["Date"] = pd.to_datetime(df["Date"])
    ...
    close_in_range   = (c - l) / rng                        # [0, 1]
    open_close_ratio = (c - o) / (o + EPS)                  # same-day return
    body_top         = np.maximum(o, c)
    upper_wick_frac  = (h - body_top) / rng                 # [0, 1]

    # 20-day trend (log-approx via pct_change)
    trend_20d = c.pct_change(20)

    # Relative volume (shifted so today's volume isn't in its own average)
    avg_vol_20 = v.rolling(20, min_periods=10).mean().shift(1)
    rel_volume  = v / (avg_vol_20 + EPS)

    # ── Step 1: Move-size extremity ───────────────────────────────────────
    move_abs  = open_close_ratio.abs()
    move_pct  = move_abs.shift(1).rolling(252, min_periods=60).rank(pct=True)
    extremity = ((move_pct - 0.75) / 0.15).clip(0, 1)

    # ── Step 2: Regime-switched candle signal ─────────────────────────────
    cir_c = close_in_range * 2 - 1                          # recentred to [-1, +1]
    core  = cir_c * (2 * extremity - 1)
    #   extremity=0  →  core = -cir_c  (high CIR = overbought → reversal)
    #   extremity=1  →  core = +cir_c  (high CIR = breakout   → continuation)

    # ── Step 3: Upper-wick z-score additive ──────────────────────────────
    uw_roll = upper_wick_frac.shift(1).rolling(252, min_periods=60)
    uw_z    = ((upper_wick_frac - uw_roll.mean()) / (uw_roll.std() + EPS)
               ).clip(-2.5, 2.5) / 2.5

    raw = core * 0.85 + uw_z * 0.15

    # ── Step 4: Trend amplitude ───────────────────────────────────────────
    trend_pct = (
        trend_20d.shift(1)
    ...
    # ── Step 5: Volume amplitude (U-shaped) ───────────────────────────────
    vol_pct = (
        rel_volume.shift(1)
        .rolling(252, min_periods=60)
        .rank(pct=True)
    ...
    u_val   = (vol_pct - 0.5) ** 2 * 4                      # 0 at median, 1 at extremes
    vol_amp = 1.0 + 0.20 * u_val                            # [1.0, 1.20]

    # ── Step 6: Amplify and normalise ─────────────────────────────────────
    amplified = raw * trend_amp * vol_amp

    roll_a = amplified.shift(1).rolling(252, min_periods=60)
    irs_v2 = (
        ((amplified - roll_a.mean()) / (roll_a.std() + EPS))
        .clip(-3, 3) / 3
    )                                                        # ∈ [-1, +1]

    df["irs_v2"]        = irs_v2
    df["irs_extremity"] = extremity
    return df
...
def calculate_volatility_cv_features(df):
    ...
    for window in [10, 20, 50]:
        ...
        # CV regime classification (only keep cv_50d regimes, 10d/20d regime features removed as weak)
        if window == 50:
...
def create_trading_signal_composite_scores(df):
    ...
    # trading_signal_composite removed (weak feature)
...
def indicators(df):
    ...
    # add_genetic_log_zscore_20d removed (weak feature)
    ...
    df = add_rolling_log_scaled_features(df, base_column, windows=[10])
    df = add_rate_of_change_features(df, base_column, periods=[3, 5, 10])
    df = add_z_score_signals(df, base_column)
    df = add_momentum_signals(df, base_column)
    # add_trend_reversal_signals removed (BullishCross/BearishCross weak)
    # add_volatility_regime_signals removed (HighVolatilityRegime weak)
    ...
    df = compute_intraday_regime_score(df)
...
def add_market_relative_features(df):
    """
    Add regime-relative features: each stock indicator minus the same indicator
    computed on SPY on the same date. These features decay slowly because they
    measure relative standing rather than absolute level.

    Safe for train/inference consistency: SPY data is available identically at
    feature-generation time and at inference time (no cross-sectional computation,
    no dependency on other tickers).
    """
    global GLOBAL_INDEX_DATA

    if not GLOBAL_INDEX_DATA or 'SPY' not in GLOBAL_INDEX_DATA:
        return df

    spy_df = GLOBAL_INDEX_DATA['SPY'].copy()
    spy_df.index = pd.to_datetime(spy_df.index)
    spy_close = spy_df['Close'].sort_index()
    spy_high  = spy_df['High'].sort_index()
    spy_low   = spy_df['Low'].sort_index()

    # SPY RSI(14)
    spy_delta    = spy_close.diff()
    spy_gain     = spy_delta.where(spy_delta > 0, 0.0)
    spy_loss     = (-spy_delta.where(spy_delta < 0, 0.0))
    spy_avg_gain = spy_gain.rolling(14, min_periods=14).mean()
    spy_avg_loss = spy_loss.rolling(14, min_periods=14).mean()
    spy_rsi      = 100.0 - (100.0 / (1.0 + spy_avg_gain / (spy_avg_loss + 1e-10)))

    # SPY momentum (% returns over N days)
    spy_mom_5d  = spy_close.pct_change(5)
    spy_mom_10d = spy_close.pct_change(10)
    spy_mom_20d = spy_close.pct_change(20)

    # SPY ATR% (14-day ATR as fraction of price)
    spy_close_prev = spy_close.shift(1)
    spy_tr = pd.concat([
        spy_high - spy_low,
        (spy_high - spy_close_prev).abs(),
        (spy_low  - spy_close_prev).abs(),
    ], axis=1).max(axis=1)
    spy_atr     = spy_tr.rolling(14, min_periods=5).mean()
    spy_atr_pct = spy_atr / (spy_close + 1e-10)

    spy_features = pd.DataFrame({
        '_spy_rsi':     spy_rsi,
        '_spy_mom_5d':  spy_mom_5d,
        '_spy_mom_10d': spy_mom_10d,
        '_spy_mom_20d': spy_mom_20d,
        '_spy_atr_pct': spy_atr_pct,
    })
    spy_features.index.name = 'Date'
    spy_features = spy_features.reset_index()
    spy_features['Date'] = pd.to_datetime(spy_features['Date'])

    result_df = df.copy()
    result_df = result_df.sort_values('Date')
    result_df = pd.merge_asof(
        result_df,
        spy_features.sort_values('Date'),
        on='Date',
        direction='backward',
    )
    ...
    if 'RSI' in result_df.columns:
        result_df['rsi_vs_spy'] = (result_df['RSI'] - result_df['_spy_rsi']).clip(-50, 50)

    # --- Momentum relative ---
    stock_mom_5d  = result_df['Close'].pct_change(5)
    stock_mom_10d = result_df['Close'].pct_change(10)
    stock_mom_20d = result_df['Close'].pct_change(20)
    result_df['momentum_5d_vs_spy']  = (stock_mom_5d  - result_df['_spy_mom_5d']).clip(-0.5, 0.5)
    result_df['momentum_10d_vs_spy'] = (stock_mom_10d - result_df['_spy_mom_10d']).clip(-0.5, 0.5)
    result_df['momentum_20d_vs_spy'] = (stock_mom_20d - result_df['_spy_mom_20d']).clip(-0.5, 0.5)

    # --- ATR% relative ---
    if 'atr_percentage' in result_df.columns:
        result_df['atr_pct_vs_spy'] = (result_df['atr_percentage'] - result_df['_spy_atr_pct']).clip(-0.2, 0.2)

    # Drop temp SPY columns
    result_df.drop(
        columns=['_spy_rsi', '_spy_mom_5d', '_spy_mom_10d', '_spy_mom_20d', '_spy_atr_pct'],
        errors='ignore',
        inplace=True,
    )
