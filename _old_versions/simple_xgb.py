...
parser.add_argument("--drop_feature_patterns", type=str, default=None,
                    help="Comma-separated substrings. Any feature whose name "
                         "contains one of these strings (case-sensitive) is "
                         "dropped before training. E.g. 'beta_IWM,HC_Predict_Regime'.")
...
def main():
    ...
    with Phase("Build feature matrix + labels"):
        ...
        if args.drop_feature_patterns:
            patterns = [p.strip() for p in args.drop_feature_patterns.split(",") if p.strip()]
            dropped_p = [c for c in base_feature_cols if any(pat in c for pat in patterns)]
            base_feature_cols = [c for c in base_feature_cols if not any(pat in c for pat in patterns)]
            logging.info(f"  --drop_feature_patterns={patterns}: removed {len(dropped_p)} cols "
                         f"(examples: {dropped_p[:8]})")
            logging.info(f"  remaining base features: {len(base_feature_cols)}")
