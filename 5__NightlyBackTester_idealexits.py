...
class StockSniperStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.trade_recorder = TradeRecorder('trade_history_idealexits.parquet')  # IDEAL-EXITS: redirect, do NOT clobber live trade_history.parquet
    ...
    def process_buy_candidates(self, buy_candidates, current_date, verbose=False):
        ...
        """
        export_today = False  # IDEAL-EXITS: disable live-signal exports (no finviz/FinBERT, no clobber of live signal files); sim P&L unaffected
    ...
    def execute_buy_with_bracket(self, data, size, current_date):
        ...
        # ===== IDEAL-EXITS EXPERIMENT (2026-06-01) =====
        # From the intraday exit study (8.2__ExitOptimizer): the live/baseline 3% trail
        # was the costliest exit (bled ~-$2.91/trade vs ride-to-close). The least-damaging
        # "must have a stop + a trailing stop" config was a WIDE 12% trail, which on a
        # StopTrail leg also doubles as the ~12% catastrophe/hard stop (initial floor =
        # entry*(1-0.12)), and effectively NO take-profit. So:
        take_profit_percent = 100.0      # non-binding -> effectively no take-profit
        trailing_stop_percent = 12.0     # wide trail == 12% catastrophe stop + primary trailing exit
        ...
        if bracket_orders:
            ...
            # update_buy_signal(symbol, current_date, current_price, data.UpProbability[0])  # IDEAL-EXITS: neutralized (writes _Buy_Signals.parquet; not used by sim P&L)
    ...
    def handle_sell_execution(self, order):
        ...
        if self.getposition(data).size == 0:
            ...
            if entry_price is not None:
                ...
                # add_completed_trade(completed_trade_data)  # IDEAL-EXITS: neutralized (appends to live Data/TradeHistory.parquet)

                is_loss_for_tracking = profit_abs < 0
                # update_trade_result(symbol, is_loss_for_tracking, exit_price, exit_date)  # IDEAL-EXITS: neutralized (writes _Buy_Signals.parquet)
...
if __name__ == "__main__":
    # clear_completed_trades()  # IDEAL-EXITS: neutralized (wipes live Data/TradeHistory.parquet)
    main()
