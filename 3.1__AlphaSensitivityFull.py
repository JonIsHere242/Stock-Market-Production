...
# 3.1__AlphaSensitivityFull.py
# Full-history variant of 3__AlphaSensitivity.py.
# Reads from Data/PriceDataFull (all available history, some tickers back to 1980).
# Writes processed features to Data/ProcessedDataFull.
# Indexes fetched via yfinance (goes to ETF inception dates) instead of 2-year IBKR pull.
import os
import pandas as pd
import numpy as np
import time
...
CONFIG = {
    'input_directory': 'Data/PriceDataFull',
    'output_directory': 'Data/ProcessedDataFull',
    'index_temp_dir': 'Data/IndexesFull',
    'log_lines_to_read': 500,
    'core_count_division': True,
}

# yfinance ticker symbols for each index (used by fetch_index in the full-history version)
YFINANCE_SYMBOLS = {
    'SPY': 'SPY',   # S&P 500 ETF — inception 1993-01-22
    'QQQ': 'QQQ',   # Nasdaq 100 ETF — inception 1999-03-10
    'IWM': 'IWM',   # Russell 2000 ETF — inception 2000-05-22
    'DIA': 'DIA',   # Dow Jones ETF — inception 1998-01-20
    'VIX': '^VIX',  # CBOE VIX index — available from 1990-01-02
}
...
logger = get_logger("Data/logging/3.1__IndicatorsFull")
...
def fetch_index(symbol, start_date=None, end_date=None, cache_dir='Data/IndexesFull'):
    """Fetch a market index via yfinance (full history to ETF inception).

    Falls back to on-disk cache when yfinance is unavailable.
    Cache is refreshed whenever the requested window extends beyond what is cached.
    """
    ...
    # Default start: go as far back as possible (yfinance returns what it has)
    ...
    if os.path.exists(cache_file):
        ...
        need_update = (
            ...
            end_date.date() > cached.index.max().date()
        )
        if not need_update:
            print(f"  {symbol}: Using cache ({len(cached)} days, "
                  f"{cached.index.min().date()} → {cached.index.max().date()})")
            return cached
        print(f"  {symbol}: Cache needs refresh — re-downloading via yfinance...")
    else:
        print(f"  {symbol}: Downloading via yfinance (full history)...")
    ...
    try:
        ...
        if raw.empty:
            raise ValueError(f"yfinance returned empty DataFrame for {yf_symbol}")
        ...
        # VIX has no meaningful volume — fill with synthetic value so
        # downstream code that expects a Volume column doesn't error
        if 'Volume' not in raw.columns or (raw['Volume'] == 0).all():
            np.random.seed(42)
            raw['Volume'] = np.random.randint(5_000_000, 15_000_000, size=len(raw))

        if 'Close' not in raw.columns:
            raise ValueError(f"No Close column in yfinance data for {yf_symbol}")

        raw['Adj Close'] = raw['Close']
        data = raw[['Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume']].copy()
        data.to_parquet(cache_file)
        print(f"    Saved {len(data)} days ({data.index.min().date()} → "
              f"{data.index.max().date()}) to {cache_file}")
        return data
    ...
    if os.path.exists(cache_file):
        print(f"    Falling back to cached data for {symbol}")
        cached = pd.read_parquet(cache_file)
        cached.index = pd.to_datetime(cached.index)
        return cached
...
def update_all_indexes(start_date=None, end_date=None):
    ...
    print("\n" + "="*70)
    print("UPDATING MARKET INDEXES (full history via yfinance)")
    print("="*70)
    ...
    for symbol, config in INDEXES.items():
        ...
        data = fetch_index(symbol, start_date, end_date,
                           cache_dir=CONFIG['index_temp_dir'])
        if data is not None:
            results[symbol] = data
            success += 1
        else:
    ...
    if results:
        print(f"\nData saved in: {CONFIG['index_temp_dir']}/")
        print("\nAvailable indexes:")
...
def load_index(symbol, cache_dir=None):
...
def load_all_indexes(cache_dir=None):
    if cache_dir is None:
        cache_dir = CONFIG['index_temp_dir']
...
def load_indexes_for_worker():
    ...
    index_dir = CONFIG['index_temp_dir']  # Data/IndexesFull in the full-history version
...
def process_data_files(run_percent):
    ...
    # Update/download all indexes via yfinance — saves to Data/IndexesFull
    print("Updating market indexes (full history via yfinance)...")
    update_all_indexes()
...
if __name__ == "__main__":
    logger.info("Starting 3.1__AlphaSensitivityFull — full-history feature generation")
    logger.info(f"  Input:   {CONFIG['input_directory']}")
    logger.info(f"  Output:  {CONFIG['output_directory']}")
    logger.info(f"  Indexes: {CONFIG['index_temp_dir']}")
    parser = argparse.ArgumentParser(
        description="Full-history feature generation from Data/PriceDataFull."
    )
    parser.add_argument(
        '--runpercent', type=int, default=100,
        help="Percentage of ticker files to process (default: 100)."
    )
