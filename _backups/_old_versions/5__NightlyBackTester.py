...
class StockSniperStrategy(bt.Strategy):
    ...
    def get_buy_candidates(self, current_date):
        ...
        # VIX regime sizing: reduce position size by 25% during high-VIX periods
        # to limit correlated drawdowns when multiple positions hit stop loss simultaneously.
        # High VIX (>= 25) shows best Sharpe (12.49) but concentrated losses spike drawdown.
        vix_size_factor = 1.0
        for d in self.datas:
            try:
                vix_val = float(d.VIX_Close[0])
                if vix_val >= 25.0:
                    vix_size_factor = 0.75
                break
        ...
        for d in self.datas:
            ...
            if self.can_buy(d, current_date):
                ...
                if vix_size_factor != 1.0:
                    size = max(0, int(size * vix_size_factor))
    ...
    def can_buy(self, data, current_date):
        """
        Optimized can_buy with EWM Z-score signal (from 10_ZScoreWeighting.ipynb, 2026-02-17).

        Core signal change: replaced percentile-only gate with EWM Z-score.
          - EWM Z-score (span=30) >= 1.5 as primary signal (PF 5.19, WR 74.3%)
          - Percentile P90 kept as secondary confirmation
          - Price Z-score divergence bonus: prob Z high + price Z < 0 is best (PF 3.70)
          - All other filters (price, vol, momentum, RSI, etc.) unchanged from param sweep

        Combined result: PF ~5.2 / WR ~74% / Sharpe ~8.8 vs old PF 2.67 / WR 66%
        ...
        EWM_SPAN = 30
        EWM_MIN_PERIODS = 10
        MIN_Z_SCORE = 1.5              # EWM Z-score threshold (from 10_ZScoreWeighting sweep)
        PRICE_Z_LOOKBACK = 45
        PRICE_Z_BONUS_THRESHOLD = 0.0  # Prob Z high + price Z below this = best signals
        ...
        MAX_SINGLE_DAY_DROP = -0.15
        RECENT_DROP_LOOKBACK_DAYS = 5

        MOMENTUM_LOOKBACK_DAYS = 5
        MAX_MOMENTUM_GAIN = 0.10
        MAX_MOMENTUM_LOSS = -0.05
        ...
        RSI_PERIOD = 14
        MIN_RSI_THRESHOLD = 35

        # Percentile fallback for limited-data stocks
        PERCENTILE_FALLBACK_P_LOW = 90.0
        PERCENTILE_FALLBACK_P_HIGH = 99
        ...
        # Probability floor
        if current_prob < UP_PROB_MIN_BOUND:
            return False
        ...
        historic_probs = []
        i = 1
        while len(historic_probs) < 60 and i < 100:
            try:
                prob_val = data.UpProbability[i]
                if prob_val is not None:
                    historic_probs.append(float(prob_val))
        ...
        # Need minimum history for Z-score computation
        if len(historic_probs) < EWM_MIN_PERIODS:
            return False

        # ==== EWM Z-SCORE COMPUTATION ====
        # Build full prob series (current + history) for EWM calculation
        # historic_probs is ordered [t-1, t-2, ...], reverse for chronological
        prob_series = list(reversed(historic_probs)) + [float(current_prob)]
        prob_arr = np.array(prob_series)

        # Compute EWM mean and std using pandas (matches notebook exactly)
        alpha = 2.0 / (EWM_SPAN + 1)
        n = len(prob_arr)

        # EWM mean: iterative computation
        ewm_mean = prob_arr[0]
        for j in range(1, n):
            ewm_mean = alpha * prob_arr[j] + (1 - alpha) * ewm_mean

        # EWM variance: iterative computation
        ewm_var = 0.0
        ewm_m = prob_arr[0]
        sum_weights = 1.0
        sum_weights_sq = 1.0
        for j in range(1, n):
            ewm_m_prev = ewm_m
            ewm_m = alpha * prob_arr[j] + (1 - alpha) * ewm_m_prev
            sum_weights = 1.0 + (1 - alpha) * sum_weights
            sum_weights_sq = 1.0 + ((1 - alpha) ** 2) * sum_weights_sq
            diff = prob_arr[j] - ewm_m_prev
            ewm_var = (1 - alpha) * (ewm_var + alpha * diff * diff)

        # Bias correction (matches pandas default adjust=True behavior)
        bias_correction = sum_weights ** 2 / (sum_weights ** 2 - sum_weights_sq)
        ewm_std = np.sqrt(ewm_var * bias_correction) if ewm_var * bias_correction > 0 else 0

        if ewm_std < 1e-10:
            prob_z_score = 0.0
        else:
            prob_z_score = (float(current_prob) - ewm_mean) / ewm_std

        # ==== Z-SCORE GATE ====
        z_passed = prob_z_score >= MIN_Z_SCORE

        # If Z-score doesn't pass, fall back to percentile for stocks with enough data
        if not z_passed:
            if len(historic_probs) >= 30:
                p_low = np.percentile(historic_probs, PERCENTILE_FALLBACK_P_LOW)
                p_high = np.percentile(historic_probs, PERCENTILE_FALLBACK_P_HIGH)
                if not (current_prob >= p_low and current_prob < p_high):
                    return False
            else:
                return False

        # ==== RECENT DROP CHECK (for all stocks) ====
        try:
            for lookback in range(1, min(RECENT_DROP_LOOKBACK_DAYS + 1, 100)):
                try:
                    prev_close = data.close[-lookback]
        ...
        # ==== PRICE Z-SCORE DIVERGENCE CHECK ====
        # Penalize signals where price has already run up (prob Z high but price Z also high)
        # Best signals: prob Z high + price Z low (model sees something price hasn't reflected)
        if z_passed and len(historic_probs) >= PRICE_Z_LOOKBACK:
            try:
                price_closes = data.close.get(size=PRICE_Z_LOOKBACK)
                if price_closes is not None and len(price_closes) >= PRICE_Z_LOOKBACK:
                    price_mean = np.mean(price_closes)
                    price_std = np.std(price_closes)
                    if price_std > 0:
                        price_z = (current_close - price_mean) / price_std
                        # If price has already spiked hard (>1.5 std), require even higher prob Z
                        # to compensate (WR drops from 71% to 61% when price Z > 1.5)
                        if price_z > 1.5 and prob_z_score < 2.0:
                            return False
            except Exception:
                pass
    ...
    def can_buy_____BACKUP(self, data, current_date):
        """
        ...
        UP_PROB_MIN_BOUND = 0.2
        UP_PROB_MAX_BOUND = 0.95          # Widened: was 0.8 (new calibration is more honest, 0.95 is meaningful now)

        MIN_CLOSE_PRICE = 2.00
        MAX_CLOSE_PRICE = 1000.00
        MIN_DOLLAR_VOLUME = 1_500_000

        MAX_SINGLE_DAY_DROP = -0.25       # Tighter: was -0.15
        ...
        SUFFICIENT_DATA_P_LOW = 80.0      # Widened: was 90 (new calibration has compressed range, need wider window)
        SUFFICIENT_DATA_P_HIGH = 98.0     # Widened: was 98 (don't cap the best signals — they're real now)
        LIMITED_DATA_P_LOW = 80           # Widened: was 90 (match sufficient data)
        LIMITED_DATA_P_HIGH = 99          # Widened: was 98
    ...
    def execute_buy_with_bracket(self, data, size, current_date):
        ...
        # ATR-adaptive trailing stop: 1.5% base + 0.75x excess ATR beyond 2%, capped at 5%
        # Matches the formula validated in 10_PredictorTest.ipynb
        atr = self.inds[data]['atr'][0] if data in self.inds and 'atr' in self.inds[data] else current_price * 0.02
        atr_pct = (atr / current_price) * 100 if current_price > 0 else 2.0
        trailing_stop_percent = min(5.0, 1.5 + 0.75 * max(0.0, atr_pct - 2.0))
        take_profit_percent = 20.0  # Wide TP; trailing stop is the primary exit
...
def extract_backtest_results(strategy, cerebro, logger):
    ...
    try:
        ...
        # Transfer strategy attributes populated by get_analyzer_data into results
        results['sqn_value'] = getattr(strategy, 'sqn_value', 0) or 0
        results['vwr'] = getattr(strategy, 'vwr', 0) or 0
        results['daily_return'] = getattr(strategy, 'daily_return', 0) or 0
        results['time_in_market_pct'] = getattr(strategy, 'time_in_market_pct', 0) or 0
        ...
        # Pass individual trade PnL list for accurate SQN
        results['_trade_history'] = getattr(strategy, 'trade_history', [])
        ...
        calculate_signal_quality_metrics(results, logger)

        results.pop('_trade_history', None)
        ...
        try:
            ...
            if key == 'sqn':
                ...
                if sqn_value is not None:
                    ...
                    # Calculate SQN from real individual trade PnLs if available
                    trade_history = getattr(strategy, 'trade_history', [])
                    if trade_history and len(trade_history) >= 2:
                        pnls = [t['PnL'] for t in trade_history if 'PnL' in t]
                        if len(pnls) >= 2:
                            mean_pnl = sum(pnls) / len(pnls)
                            variance = sum((p - mean_pnl) ** 2 for p in pnls) / len(pnls)
                            std_pnl = variance ** 0.5
                            if std_pnl > 0:
                                strategy.sqn_value = (mean_pnl / std_pnl) * (len(pnls) ** 0.5)
                                logger.info(f"SQN from trade history ({len(pnls)} trades): {strategy.sqn_value:.2f}")
            elif key == 'returns':
...
def process_daily_returns_data(results, analyzer_data, logger):
    ...
    try:
        ...
        # annualized_return is in % (e.g. 82.0); time_return is decimal — multiply deviation by 100
        ...
        # daily_returns are decimal; cumsum gives cumulative decimal return; no /100 needed
        equity_curve = [results['initial_value'] * (1 + r) for r in np.cumsum(daily_returns)]
        drawdowns = []
        peak = equity_curve[0]
        ...
        if len(daily_returns) > 5:
            ...
            # Tail Ratio = P95 of daily returns / |P5 of daily returns|
            p95 = np.percentile(daily_returns, 95)
            p5 = np.percentile(daily_returns, 5)
            if p5 < 0:
                tail_ratio = p95 / abs(p5)
                results['tail_ratio'] = tail_ratio
                if results.get('profit_factor', 0) > 0:
                    results['common_sense_ratio'] = results['profit_factor'] * tail_ratio

            # Probabilistic Sharpe Ratio (Bailey & Lopez de Prado, 2012)
            # P(true annualized SR > 1.0), adjusted for skewness and kurtosis
            if len(daily_returns) > 20:
                dr_arr = np.array(daily_returns)
                sr_hat = np.mean(dr_arr) / np.std(dr_arr, ddof=1)  # daily SR
                sr_star = 1.0 / np.sqrt(252)                        # daily equiv of annualized 1.0
                gamma3 = stats.skew(dr_arr)
                gamma4 = stats.kurtosis(dr_arr, fisher=False)       # raw kurtosis (normal distribution = 3)
        ...
        # Rewards fast recovery and penalises lingering drawdowns, unlike Calmar's single-event max-DD
...
def process_daily_returns_data_corrected(results, analyzer_data, logger):
    ...
    try:
        ...
        if len(daily_returns) > 5:
            ...
            p95 = np.percentile(daily_returns, 95)
            p5 = np.percentile(daily_returns, 5)
            if p5 < 0 and results.get('profit_factor', 0) > 0:
                tail_ratio = p95 / abs(p5)
                results['common_sense_ratio'] = results['profit_factor'] * tail_ratio
...
def calculate_signal_quality_metrics(results, logger):
    """Placeholder — IC/IC_IR/Monotonicity removed: UpProbability is a filter threshold,
    not a continuous ranked signal, so those metrics are not meaningful for this system."""
    pass
...
def print_detailed_results(results, execution_time):
    ...
    # Compute capture ratios early so they appear in Risk Metrics
    ...
    # Monthly/yearly performance + Up/Down Capture (computed and printed here)
...
def print_risk_metrics(results):
    ...
    # Market Capture Ratios — computed before printing by _compute_capture_ratios
    ...
    if results.get('down_capture') is not None:
        # Negative = gains when market falls; unicorn triggers at <= 50/10000 ≈ 0.005
...
def print_trade_statistics(results):
    ...
    # Dollar thresholds scale with account size so a $10K account isn't punished for small positions
    ...
    print(colorize_output(results['avg_win_pct'], "Avg Win (% of Capital):", 0.4, 0.15))
    print(colorize_output(results['avg_loss_pct'], "Avg Loss (% of Capital):", results['avg_win_pct'] * 0.8, results['avg_win_pct'] * 1.1, lower_is_better=True))
