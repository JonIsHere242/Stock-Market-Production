...
def optuna_tune(X_train, y_train, dates, n_trials=50, n_cv_splits=4):
    ...
    def objective(trial):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(cv_splits):
            ...
            # Mirror production threshold-finder: sweep the full PR curve and find the
            # best precision*sqrt(coverage) subject to precision>=0.65 and coverage>=0.002.
            # 0.65 is slightly relaxed from the production 0.68 — CV folds are noisier.
            MIN_PREC_CV = 0.65
            MIN_COV_CV  = 0.002

            prec_arr, _, thresh_arr = precision_recall_curve(y_val_fold, proba, pos_label=1)
            if len(prec_arr) > len(thresh_arr):
                prec_arr = prec_arr[:-1]

            proba_sorted = np.sort(proba)
            n_proba = len(proba_sorted)
            cov_arr = (n_proba - np.searchsorted(proba_sorted, thresh_arr)) / n_proba

            valid = (prec_arr >= MIN_PREC_CV) & (cov_arr >= MIN_COV_CV)
            if valid.any():
                scores.append(float((prec_arr[valid] * np.sqrt(cov_arr[valid])).max()))
            else:
                cov_ok = cov_arr >= MIN_COV_CV
                scores.append(float(prec_arr[cov_ok].max() * np.sqrt(MIN_COV_CV)) if cov_ok.any() else 0.0)
...
def train_model(training_data, calibration_data, config, target_precision=0.75):
    ...
    # Long-only: find a high-precision threshold for UP moves only
    target_min_precision_pos = 0.68
    min_predictions_percent_pos = 0.002
    ...
    if np.any(valid_indices):
        ...
        # Step down precision floor until both constraints are satisfiable
        found = False
        for reduced_precision in [0.66, 0.64]:
            ri = (precisions_pos >= reduced_precision) & (np.array(prediction_coverage) >= min_predictions_percent_pos)
            if np.any(ri):
                cov_arr = np.array(prediction_coverage)[ri]
                bs = precisions_pos[ri] * np.sqrt(cov_arr)
                bi = np.argmax(bs)
                ri_pos = np.where(ri)[0]
                optimal_threshold_pos = thresholds_pos[ri_pos[bi]]
                pos_precision = precisions_pos[ri_pos[bi]]
                pos_recall = recalls_pos[ri_pos[bi]]
                pos_coverage = prediction_coverage[ri_pos[bi]]
                logging.info(f"UP threshold found with relaxed precision floor {reduced_precision:.2f}: "
                             f"threshold={optimal_threshold_pos:.4f}, precision={pos_precision:.3f}, "
