...
def calculate_vix_features22222(df):    
    result_df = df.copy()
...
def calculate_dvamr_probability2222(df, 
                               lookback_window=60, 
                               momentum_window=20, 
...
def Vol_estimators(df, window=21):
    """
    Calculate top-performing volatility estimators based on EDA results
    Returns dataframe with added volatility estimator columns
    """
    ...
    hl_component = 0.5 * ((np.log(high / low)) ** 2) / (4 * math.log(2))
    cc_returns = np.log(close / close.shift(1))
    cc_component = (2 * math.log(2) - 1) * (cc_returns ** 2)
    oc_component = 0.5 * (np.log(open_price / close)) ** 2
    gkyz_variance = hl_component + cc_component + oc_component
    result_df['GKYZ_Alt'] = np.sqrt(gkyz_variance.rolling(window).mean())
    
    hl = np.log(high / low)
    result_df['Parkinson'] = np.sqrt((1.0 / (4.0 * math.log(2))) * (hl ** 2).rolling(window).mean())
    
    term1 = 0.5 * (np.log(high / low)) ** 2
    term2 = (2 * math.log(2) - 1) * (np.log(close / open_price)) ** 2
    result_df['GarmanKlass'] = np.sqrt((term1 - term2).rolling(window).mean())
    
    hl_squared = (np.log(high / low)) ** 2
    sigma_hl2 = hl_squared / (4 * math.log(2))
    close_returns = np.log(close / close.shift(1))
    sigma_cc2 = close_returns ** 2
    open_close_returns = np.log(close / open_price)
    sigma_oc2 = open_close_returns ** 2
    gkyz_variance_orig = 0.5 * sigma_hl2 + (2 * math.log(2) - 1) * sigma_cc2 + sigma_oc2
    result_df['GKYZ'] = np.sqrt(gkyz_variance_orig.rolling(window).mean())

    for base_col in ['GKYZ_Alt', 'Parkinson']:
        rolling_min = result_df[base_col].rolling(50).min()
        rolling_max = result_df[base_col].rolling(50).max()
        result_df[f'{base_col}_normalized'] = (result_df[base_col] - rolling_min) / (rolling_max - rolling_min + 1e-10)
        
        result_df[f'{base_col}_high_vol_regime'] = (result_df[base_col] > result_df[base_col].rolling(50).quantile(0.7)).astype(int)
        result_df[f'{base_col}_low_vol_regime'] = (result_df[base_col] < result_df[base_col].rolling(50).quantile(0.3)).astype(int)
    
    result_df['GKYZ_Alt_ROC_5'] = result_df['GKYZ_Alt'].pct_change(5)
    result_df['GKYZ_Alt_ROC_10'] = result_df['GKYZ_Alt'].pct_change(10)
    
    result_df['vol_spread_GKYZ_Parkinson'] = result_df['GKYZ_Alt'] - result_df['Parkinson']
    result_df['vol_ratio_GKYZ_Parkinson'] = result_df['GKYZ_Alt'] / (result_df['Parkinson'] + 1e-10)
...
def add_ml_gap_indicator_minimal(df):
    """
    Minimal version - adds only ML_Style_Weighted indicator to OHLCV data
    """
    df = df.copy()
    
    # Core ratio
    hc_ratio = (df['High'] - df['Close']) / (df['Close'] + 1e-10)
    lo_ratio = (df['Low'] - df['Open']) / (df['Open'] + 1e-10)
    lo_hc_ratio = lo_ratio * hc_ratio
    
    # Gap features
    overnight_gap = (df['Open'] - df['Close'].shift(1)) / (df['Close'].shift(1) + 1e-10)
    abs_gap = abs(overnight_gap)
    avg_abs_gap = abs_gap.rolling(14, min_periods=1).mean()
    gap_size_ratio = abs_gap / (avg_abs_gap + 1e-10)
    
    # Size weight
    size_weight = 1.0
    size_weight = np.where(gap_size_ratio < 0.5, 0.7, size_weight)
    size_weight = np.where((gap_size_ratio >= 0.5) & (gap_size_ratio < 1.0), 0.9, size_weight)
    size_weight = np.where((gap_size_ratio >= 1.0) & (gap_size_ratio < 2.0), 1.3, size_weight)
    size_weight = np.where(gap_size_ratio >= 2.0, 0.8, size_weight)
    
    # Direction weight
    direction_weight = np.where(overnight_gap > 0, 1.2, 1.0)
    direction_weight = np.where(overnight_gap < 0, 0.8, direction_weight)
    
    # Frequency weight
    gap_count = (abs_gap > avg_abs_gap).rolling(10, min_periods=1).sum()
    frequency_weight = 0.8 + (gap_count * 0.05)
    
    # Recency weight (simplified)
    def simple_days_since_gap(gap_series):
        days_since = []
        last_gap = -1
        for i, val in enumerate(gap_series):
            if val > gap_series.median():
                last_gap = i
            days_since.append(i - last_gap if last_gap != -1 else 999)
        return pd.Series(days_since, index=gap_series.index)
    
    days_since_gap = simple_days_since_gap(abs_gap)
    recency_weight = np.where(days_since_gap >= 7, 1.2, 1.0)
    
    # Volume weight
    volume_ma = df['Volume'].rolling(20, min_periods=1).mean()
    volume_ratio = df['Volume'] / (volume_ma + 1e-10)
    volume_weight = np.clip((volume_ratio - 0.5) * 2, 0, 1)
    
    # Volatility weight
    volatility = df['Close'].pct_change().rolling(20, min_periods=1).std()
    vol_median = volatility.rolling(50, min_periods=1).median()
    vol_weight = np.tanh((volatility / (vol_median + 1e-10) - 1) * 2)
    vol_weight = np.clip(vol_weight, 0, 1)
    
    # Trend weight
    momentum = df['Close'].pct_change(20)
    trend_weight = np.clip(-momentum * 10, 0, 1)
    
    # ML Style Weighted combination
    df['ML_Style_Weighted'] = (
        lo_hc_ratio * 
        (size_weight ** 1.2) *
        (direction_weight ** 1.5) *
        (frequency_weight ** 0.8) *
        (recency_weight ** 0.7) *
        ((0.6 + 0.4 * volume_weight) ** 0.9) *
        ((0.3 + 0.7 * vol_weight) ** 1.1) *
        ((0.4 + 0.6 * trend_weight) ** 0.8)
    )
    
    df['ML_Style_Weighted'] = df['ML_Style_Weighted'] * 1000
...
def indicators(df):
    ...
    #df = Vol_estimators(df)
    #df = add_ml_gap_indicator_minimal(df)
