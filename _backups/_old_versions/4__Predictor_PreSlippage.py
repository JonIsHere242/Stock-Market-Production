...
def optuna_tune(X_train, y_train, dates, n_trials=50, n_cv_splits=3):
    ...
    def objective(trial):
        ...
        for train_idx, val_idx in cv_splits:
            ...
            # Profit-aligned metric: precision * sqrt(coverage) at the operating threshold.
            # Matches the threshold selector's balanced_score so Optuna optimises the same
            # objective that the strategy actually uses — tail precision with enough coverage.
            operating_threshold = 0.55  # lowered from 0.65 — achievable with compressed distributions
            preds = proba >= operating_threshold
            if preds.sum() == 0:
                scores.append(0.0)
                continue
            precision = (y_val_fold.values[preds] == 1).mean()
            coverage  = preds.mean()
            scores.append(float(precision * np.sqrt(coverage)))

        return np.mean(scores)

    study = optuna.create_study(direction='maximize', study_name='xgboost_profit_aligned')
