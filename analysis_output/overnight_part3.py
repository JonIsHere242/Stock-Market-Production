...
ROOT = "Data/_refit_allnight"      # single-seed models live here (reuse s1,s2)
ENS = "Data/_ensemble"
os.makedirs(ENS, exist_ok=True)
BACKTESTER = "5__NightlyBackTester_oosvalidate.py"
CUTOFF = "2025-12-31"
SEEDS = list(range(1, 17))          # PART 3: extend to 16 seeds (prod params)
ENSEMBLE_SIZES = [12, 16]           # only the NEW larger ensembles (2/4/8 done in PART 1)
TOP_FRAC = 0.01
PARALLEL = 3
PROD = ["--n_estimators", "304", "--max_depth", "8", "--learning_rate", "0.0509",
        "--min_child_weight", "15", "--subsample", "0.886",
        "--colsample_bytree", "0.444", "--reg_alpha", "0.0048", "--reg_lambda", "0.0223"]
...
# ---- Phase C: backtest ONLY the new larger ensembles (singles done in PART 1) ----
...
lines += ["", "THESIS: ensemble sharpe > single-seed mean sharpe, ensemble maxDD < mean,",
          ...
          "=" * 86]
rep = "\n".join(lines)
with open(f"{ENS}/ENSEMBLE_REPORT_part3.txt", "w") as f:
    f.write(rep)
print("\n" + rep, flush=True)
log(f"PART 3 DONE {(time.time()-t0)/60:.1f}m -> {ENS}/ENSEMBLE_REPORT_part3.txt")
