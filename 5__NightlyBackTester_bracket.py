...
def _inner_workers():
    ...
    v = os.environ.get('BT_INNER_WORKERS')
    if v:
        try:
            return max(1, int(v))
        except ValueError:
...
# --- exit-sweep parameters (set from CLI in main(); defaults = the good bracket) ---
SWEEP_STOP_MODE  = 'hard'    # 'hard' | 'trail'
SWEEP_STOP_PCT   = 5.0       # hard-stop distance % OR trail %
SWEEP_TP_PCT     = 3.5       # take-profit %
SWEEP_TRADE_FILE = 'Data/TradeHistory_sweep_default.parquet'
...
class StockSniperStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.trade_recorder = TradeRecorder(SWEEP_TRADE_FILE)
    ...
    def can_buy(self, data, current_date):
        ...
        SUFFICIENT_DATA_P_LOW = float(os.environ.get('BT_PLOW', 65.0))     # env-gated for param-stress; default 65.0
        SUFFICIENT_DATA_P_HIGH = float(os.environ.get('BT_PHIGH', 98.0))   # env-gated for param-stress; default 98.0
    ...
    def process_buy_candidates(self, buy_candidates, current_date, verbose=False):
        ...
        if False and buy_candidates and current_date == self.last_trading_date:  # bracket-copy: live-signal export DISABLED (no finviz/0__signals write)
            if verbose:
    ...
    def execute_buy_with_bracket(self, data, size, current_date):
        ...
        # EXIT-SWEEP PARAMS (set from CLI globals): SWEEP_STOP_MODE hard|trail, SWEEP_STOP_PCT, SWEEP_TP_PCT.
        take_profit_percent   = SWEEP_TP_PCT
        trailing_stop_percent = SWEEP_STOP_PCT if SWEEP_STOP_MODE == 'trail' else 0.0
        target_price = current_price * (1 + take_profit_percent / 100.0)

        if SWEEP_STOP_MODE == 'trail':
            logging.info(f"BUY BRACKET {symbol}: Entry=${current_price:.2f}, Size={size}, "
                        f"Trail={SWEEP_STOP_PCT}%, TP={take_profit_percent}% (${target_price:.2f})")
            bracket_orders = self.buy_bracket(
                data=data, size=size, price=current_price, exectype=bt.Order.Market,
                stopexec=bt.Order.StopTrail, trailpercent=SWEEP_STOP_PCT / 100.0,
                limitprice=target_price, limitexec=bt.Order.Limit
            )
        else:  # fixed hard stop, no trail
            stop_price = current_price * (1 - SWEEP_STOP_PCT / 100.0)
            logging.info(f"BUY BRACKET {symbol}: Entry=${current_price:.2f}, Size={size}, "
                        f"HardStop={SWEEP_STOP_PCT}% (${stop_price:.2f}), TP={take_profit_percent}% (${target_price:.2f})")
            bracket_orders = self.buy_bracket(
                data=data, size=size, price=current_price, exectype=bt.Order.Market,
                stopexec=bt.Order.Stop, stopprice=stop_price,
                limitprice=target_price, limitexec=bt.Order.Limit
            )
...
def save_guaranteed_signals_to_parquet(signals, next_trading_day=None):
    ...
    try:
        ...
        STOP_LOSS_PERCENT = float(os.environ.get('BT_STOP_PCT', 2.0))   # env-gated for param-stress; default 2.0  
...
def main():
    ...
    try:
        ...
        # Wire exit-sweep params from CLI into the module globals the strategy reads.
        global SWEEP_STOP_MODE, SWEEP_STOP_PCT, SWEEP_TP_PCT, SWEEP_TRADE_FILE
        SWEEP_STOP_MODE  = args.stop_mode
        SWEEP_STOP_PCT   = args.stop_pct
        SWEEP_TP_PCT     = args.tp_pct
        SWEEP_TRADE_FILE = f"Data/TradeHistory_sweep_{args.run_tag}.parquet"
        logger.info(f"EXIT SWEEP CONFIG: mode={SWEEP_STOP_MODE} stop={SWEEP_STOP_PCT}% tp={SWEEP_TP_PCT}% -> {SWEEP_TRADE_FILE}")
...
def print_trade_history_extremes(results):
    ...
    print("\nDeviation From Normal Trading (how wild the extremes really were):")
    print(f"{'Daily baseline:':<30}{GREY}mean {daily_mean * 100:+.2f}%/day, "
          f"1 sigma {daily_std * 100:.2f}%/day, typical move {daily_typ * 100:.2f}%{RESET}")
    print(f"{GREY}Sigma is length-adjusted (scaled by sqrt of each period's trading days),"
          f" so 3-, 4-{RESET}")
    print(f"{GREY}and 5-day weeks are compared on equal footing.{RESET}")
    if daily_std <= 0:
        return
    for name, s, cnt, fmt_date, prefix, show in periods:
        if s.empty:
...
def arg_parser():
    ...
    parser.add_argument("--stop_mode", choices=['hard','trail'], default='hard', help="EXIT SWEEP: stop type")
    parser.add_argument("--stop_pct", type=float, default=5.0, help="EXIT SWEEP: hard-stop distance %% or trail %%")
    parser.add_argument("--tp_pct", type=float, default=3.5, help="EXIT SWEEP: take-profit %%")
    parser.add_argument("--run_tag", type=str, default='default', help="EXIT SWEEP: tag for Data/TradeHistory_sweep_<tag>.parquet")
