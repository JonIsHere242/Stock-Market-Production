"""4.1__Predictor.py — SEED-ENSEMBLE production pipeline (bigger/better 4__Predictor).

Drop-in superset of 4__Predictor.py: identical CLI/flags and identical output
(per-ticker RFpredictions parquets, same UpProbability/UpPrediction schema). The
ONLY difference: instead of one XGBClassifier it trains `--n_seeds` (default 8)
diverse members and AVERAGES their predict_proba at inference, then ranks/rescales
exactly as before. Averaging cancels the model's seed variance by construction.

Validated 2026-06-04 (strategy-level backtest, Jan-Apr 2026 OOS): vs the single-seed
distribution (ann mean 178%, Sharpe 2.79, maxDD 21.6%; range 87-266% / 1.74-3.54),
the ensembles land high and stable and maxDD FALLS MONOTONICALLY with size:
  ens4  247% / 3.30 / 18.8     ens8  224% / 3.32 / 18.8
  ens12 228% / 3.27 / 17.3     ens16 254% / 3.63 / 16.8
=> kills the seed lottery, lifts Sharpe ~2.79->~3.4, cuts drawdown ~21.6->~17%.
Set --n_seeds 1 to reproduce 4__Predictor.py exactly. Usage is unchanged:
  python 4.1__Predictor.py                 # train 8-seed ensemble + inference
  python 4.1__Predictor.py --predict_only  # (alias --predict) inference from saved ensemble
  python 4.1__Predictor.py --no_tune --n_seeds 16
...
# Ensemble (4.1 = seed-ensemble production model)
parser.add_argument("--n_seeds", type=int, default=8, help="Number of XGB members in the seed-ensemble (averaged at inference). 1 = single model (== 4__Predictor.py). 8 default; 16 marginally better at 2x train cost. Validated 2026-06-04: ens cuts seed variance, lifts Sharpe + lowers drawdown vs a single seed.")
...
# Modes
parser.add_argument("--predict_only", "--predict", dest="predict_only", action="store_true", help="Skip training. Load saved ensemble and run inference only (averages the N members). --predict is an alias.")
...
def train_model(X_train, y_train, tuned_params, sw_train, scale_pos_weight,
                ...
                early_stopping_rounds, random_state=42, verbose_log=True):
    """Fit one XGBClassifier on full training data.

    Uses last 20% of train as internal val for early stopping.
    `random_state` varies per ensemble member (different subsample/colsample
    draws -> a diverse model), which is what the seed-ensemble averages over.
    """
...
# -------------------------------------------------------------------------- #
# Seed-ensemble: average predict_proba over N diverse members                #
# -------------------------------------------------------------------------- #
class EnsembleModel:
    """Bag of XGBClassifiers; predict_proba is the mean across members.

    Drop-in for a single XGBClassifier everywhere downstream: exposes
    feature_names_in_, best_iteration, feature_importances_ so evaluate_and_save
    and run_inference work unchanged. Averaging the per-member scores reduces the
    model's seed variance by construction (validated 2026-06-04: ann 178->~240,
    Sharpe 2.79->~3.4, maxDD 21.6%->~17% vs the single-seed mean)."""

    def __init__(self, models):
        self.models = list(models)
        first = self.models[0]
        self.feature_names_in_ = first.feature_names_in_
        self.best_iteration = int(np.mean([m.best_iteration for m in self.models]))
...
def train_ensemble(X_train, y_train, tuned_params, sw_train, n_seeds, base_seed,
                   **train_kwargs):
    """Train `n_seeds` models that differ only by random_state, return an
    EnsembleModel. Optuna (if used) runs once upstream; all members share the
    tuned params and differ via XGB's stochastic row/column subsampling."""
    models = []
    for k in range(n_seeds):
        clf = train_model(X_train, y_train, tuned_params, sw_train,
                          random_state=base_seed + k,
                          verbose_log=(k == 0), **train_kwargs)
        models.append(clf)
        logging.info(f"  ensemble member {k+1}/{n_seeds}: "
                     f"{clf.best_iteration+1} trees")
    ens = EnsembleModel(models)
    logging.info(f"  ENSEMBLE of {n_seeds} members built "
                 f"(mean {ens.best_iteration+1} trees/member)")
    return ens
...
def evaluate_and_save(clf, X_calib, y_calib, calib_df, feature_cols,
    ...
    # Save model. For an ensemble, persist the plain list of members (standard
    # XGBClassifier pickles — no custom class in the file, so it loads anywhere);
    # run_inference reconstructs the EnsembleModel wrapper on load.
    dump(clf.models if isinstance(clf, EnsembleModel) else clf, model_path)
...
def run_inference(model_path, input_dir, output_dir, date_col, ticker_col,
    ...
    with Phase("Load saved model"):
        ...
        if isinstance(clf, list):          # ensemble: list of members -> wrap
            clf = EnsembleModel(clf)
            logging.info(f"  loaded {clf.n_members}-member ensemble")
...
def main():
    ...
    if not args.predict_only:
        ...
        # Phase 10: Train the seed-ensemble (N members, averaged at inference)
        with Phase(f"Phase 10: Train {args.n_seeds}-seed XGB ensemble"):
            clf = train_ensemble(
