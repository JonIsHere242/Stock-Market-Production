...
config = {
    ...
    "xgb_params": {
        ...
        "random_state": 3301,
        "verbosity": 2,
        "nthread": 32,  # Number of threads for parallel processing
    },
...
def train_model(training_data, calibration_data, config, target_precision=0.75):
    ...
    # Split training data for validation (used for early stopping)
    ...
    # Get predicted probabilities using the appropriate model
    y_pred_proba = clf_for_prediction.predict_proba(X_calib)
    ...
    if np.any(valid_indices):
        ...
        if valid_thresholds:
            ...
            if predicted_pos.sum() > 0:
                pos_precision = (y_calib[predicted_pos] == 1).mean()
            else:
                pos_precision = 0
            pos_recall = (predicted_pos & (y_calib == 1)).sum() / (y_calib == 1).sum() if (y_calib == 1).sum() > 0 else 0
    ...
    precisions_neg, recalls_neg, thresholds_neg = precision_recall_curve(
        1 - y_calib, y_pred_proba[:, 0], pos_label=1
    )
    ...
    if np.any(valid_indices):
        ...
        if np.any(valid_indices):
            ...
            if predicted_neg.sum() > 0:
                neg_precision = ((1 - y_calib)[predicted_neg] == 1).mean()
            else:
                neg_precision = 0
            neg_recall = (predicted_neg & ((1 - y_calib) == 1)).sum() / ((1 - y_calib) == 1).sum() if ((1 - y_calib) == 1).sum() > 0 else 0
